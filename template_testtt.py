...
class TestApiDataRetriever:
    ...
    def test_get_historical_data(self, mock_get):
        # First call to get_weather_data returns mock_weather_data
        mock_get.side_effect = [
            MagicMock(json=lambda: mock_weather_data),   # First call returns mock_weather_data
            MagicMock(json=lambda: mock_historical_data) # Second call returns mock_historical_data
            ]

        # testing a class that calls requests.get multiple times within the same 
        # method (e.g., once in get_weather_data and once or more in get_historical_data).
    ...
    def test_get_json_info(self, mock_get):
        # First call to get_weather_data and subsequent calls to get_historical_data
        mock_get.side_effect = [
            MagicMock(json=lambda: mock_weather_data),
            MagicMock(json=lambda: mock_historical_data),
            MagicMock(json=lambda: mock_historical_data)
        ]
        # When you mock a function like requests.get, you want to control what it 
        # returns when it's called in your tests. side_effect allows you to define a 
        # sequence of responses for multiple calls to that function.
